        ...
        if audio_tower is not None and (input_ids.shape[1] != 1 or self.training) and audios is not None:
            ...
            with torch.no_grad():
                ...
                for audios_list in audios:
                    if len(audios_list) == 0:
                        dummy_audio_feature = torch.zeros(self.config.audio_token_len, self.config.mm_hidden_size,
                        ...
                        for audio in audios_list:
                            decoder_input_ids = torch.ones(
                                (1, self.config.audio_token_len)) * audio_tower.config.decoder_start_token_id
            ...
            for cur_input_ids, cur_input_embeds, cur_audio_features in zip(input_ids, inputs_embeds, bs_audio_features):
                if (cur_input_ids == audio_config.audio_patch_token).sum() == 0:
                    ...
                    continue
                if (cur_input_ids == audio_config.audio_start_token).sum() != (
                        cur_input_ids == audio_config.audio_end_token).sum():
                ...
                if len(audio_start_tokens) != len(cur_audio_features):
                    raise ValueError(
                        f"The number of audio start tokens ({len(audio_start_tokens)}) and audio features ({len(cur_audio_features)}) should be the same.")
                    ...
                    if orig_embeds_params is not None:
                        cur_new_input_embeds = torch.cat(
                            (cur_input_embeds[:audio_start_token_pos].detach(),
                             ...
                             cur_audio_feature,
                             cur_input_embeds[
                             audio_start_token_pos + num_patches + 1:audio_start_token_pos + num_patches + 2],
...
class LlaaaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def initialize_audio_tokenizer(self, tokenizer, device,
        ...
        num_new_tokens += tokenizer.add_tokens([DEFAULT_AUDIO_START_TOKEN, DEFAULT_AUDIO_END_TOKEN],
                                               special_tokens=True)
        self.resize_token_embeddings(len(tokenizer))
        ...
        audio_start_token, audio_end_token = tokenizer.convert_tokens_to_ids(
